...
def rgbd_slam(config: dict):
    ...
    # # get pcd_file
    # for time_idx in tqdm(range(0, 2000)):
    #     pcd = get_Lnet_data(dataset, time_idx, config['scene_radius_depth_ratio'], config['mean_sq_dist_method'])

    # # get gt_pose file
    # poses = []
    # with open('./data/Replica/room0/traj.txt', "r") as f:
    #     lines = f.readlines()
    # for time_idx in tqdm(range(0, 2000)):
    #     line = lines[time_idx]
    #     gt_pose = np.array(list(map(float, line.split()))).reshape(4, 4)
    #     gt_pose = torch.from_numpy(gt_pose).float()

    #     rel_rot = gt_pose[:3, :3].unsqueeze(0).detach()
    #     rel_rot_quat = matrix_to_quaternion(rel_rot).reshape(1,4)
    #     rel_tran = gt_pose[:3, 3].detach().reshape(1,3)
    #     gt_pose = torch.cat((rel_tran, rel_rot_quat), dim=1).detach().cpu().numpy() 
    ...
    # print(poses.shape)
    # np.save("./pcd_save/replica/pcd/gt_pose.npy", poses)
    ...
    # for time_idx in tqdm(range(0, number_of_frames, 3)):
    ...
    # num_keyframes = 5
    # for time_idx in tqdm(range(0, number_of_frames)):
    #     # Load RGBD frames incrementally instead of all frames
    #     color, depth, _, gt_pose = dataset[time_idx]
    #     # Process poses
    #     gt_w2c = torch.linalg.inv(gt_pose)
    ...
    #     id = time_idx
    #     selected_keyframes = keyframe_selection_overlap(id, depth, curr_w2c, intrinsics, keyframe_list, num_keyframes)
    #     selected_time_idx = [keyframe_list[frame_idx]['id'] for frame_idx in selected_keyframes]
    #     if id == 0:
    #         selected_time_idx = np.array([0]+[1]+[2]+[3]+[4]+selected_time_idx)
    #     elif id == 1:
    #         selected_time_idx = np.array([1]+[0]+[2]+[3]+[4]+selected_time_idx)
    #     elif id == 1998:
    #         selected_time_idx = np.array([1998]+[1997]+[1999]+[1996]+[1995]+selected_time_idx)    
    #     elif id == 1999:
    #         selected_time_idx = np.array([1999]+[1997]+[1998]+[1996]+[1995]+selected_time_idx)        
    #     else:
    #         selected_time_idx = np.array([id]+[id-1]+[id-2]+[id+1]+[id+2]+selected_time_idx)
    #     print('frame[', time_idx, ']:', selected_time_idx)
    #     group_matrix.append(selected_time_idx)

    # group_matrix = np.concatenate(group_matrix, axis=0).reshape(number_of_frames,10)
    # print(group_matrix.shape)
    # np.save("./pcd_save/replica/prior/group_matrix.npy", group_matrix)
    ...
    if config['load_checkpoint']:
        ...
        params = dict(np.load(load_path, allow_pickle=True))
    ...
    # # Save as npy
    # np.save(f"./pcd_save/{config['run_name']}/full_group_matrix_12595.npy", full_group_matrix)
    full_group_matrix = np.load(f"./pcd_save/{config['run_name']}/full_group_matrix_2000.npy")
    print('full_group_matrix:', full_group_matrix.shape)

    # print('visualize 2d gt pose ...')
    # gt_pose = 
    # for()

    print('load model')
    save_dir = os.path.join('./pcd_save', config['run_name'], f"better_model_2000.pth")
                    ...
                    if time_idx > 0 and not config['tracking']['use_gt_poses']:
                        ...
                        if time_idx == 1 and epoch == -1:
                            for init_epoch in tqdm(range(100)):
                                ...
                                #     loss, variables, losses = model(obs_local, params, group_matrix, time_idx, tracking_curr_data, variables)
                                #     print(loss)
                                ...
                                # scaler.update()
                                loss_init, flag, loss = model(obs_local, params, group_matrix, time_idx, group_data, variables, loss_init=loss_init, flag=flag)
                        ...
                        #     loss, variables, losses = get_loss(params, tracking_curr_data, variables, iter_time_idx, config['tracking']['loss_weights'], config['tracking']['use_sil_for_loss'], config['tracking']['sil_thres'],
        ...
        if GSupdate:
            ...
            for test_time_idx in tqdm(range(0, num_frames)):
                ...
                with torch.no_grad():
                    ...
                    Lnet_pose_est.append(est_pose.cpu().numpy())
                    Lnet_pose_est[-1] = Lnet_pose_est[-1].reshape(-1)
            ...
            for time_idx in tqdm(range(num_frames)):
                ...
                num_keyframes = 15
                selected_keyframes = full_group_matrix[time_idx, :num_keyframes]
                ...
                print(f"\nSelected Keyframes at Frame {time_idx}: {selected_keyframes}")
