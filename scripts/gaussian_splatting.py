...
from utils.common_utils import seed_everything, save_seq_params, save_params
...
def add_new_gaussians(params, variables, curr_data, sil_thres, time_idx, mean_sq_dist_method):
    ...
    if torch.sum(non_presence_mask) > 0:
        ...
        variables['timestep'] = torch.zeros(num_pts, device="cuda").float()
...
def rgbd_slam(config: dict):
    ...
    # Load all RGBD frames - online dataloader
    # color_all_frames = []
    # depth_all_frames = []
    gt_w2c_all_frames = []
    gs_cams_all_frames = []
