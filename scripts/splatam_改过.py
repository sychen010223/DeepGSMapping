...
def rgbd_slam(config: dict):
    ...
    # Iterate over Scan
    for time_idx in tqdm(range(checkpoint_time_idx-1, num_frames)):
        torch.cuda.device(1)
        ...
        if time_idx == 0 or (time_idx+1) % config['map_every'] == 0:
            ...
            with torch.no_grad():
                ...
                # num_keyframes = len(keyframe_list[:-1])
        ...
        if (time_idx+1) % config["checkpoint_interval"] == 0 and config['save_checkpoints']:
            ckpt_output_dir = os.path.join(config["workdir"], config["run_name"])
            save_params_ckpt(params, ckpt_output_dir, (time_idx+1))
            np.save(os.path.join(ckpt_output_dir, f"keyframe_time_indices{(time_idx+1)}.npy"), np.array(keyframe_time_indices))
